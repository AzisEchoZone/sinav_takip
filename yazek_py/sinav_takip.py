...
ana_pencere = tk.Tk()
ana_pencere.title("SINAV TAKİP PROGRAMI")
ana_pencere.geometry("1000x700")


def tarih_formatla(tarih_metni):
    try:
        return datetime.strptime(tarih_metni, "%Y-%m-%d").date()  
    except ValueError:
        return None  
...
ttk.Label(form_cercevesi, text="Büt:").grid(row=3, column=0, padx=10, pady=5)
but_alani = ttk.Entry(form_cercevesi)
but_alani.grid(row=3, column=1, padx=10, pady=5)
...
def sinav_ekle():
    ...
    but = but_alani.get() or None

    if not ders:
        messagebox.showwarning("Uyarı", "Ders alanını doldurunuz.")
        return

    db.execute("INSERT INTO sinavlar(ders, tarih, vize, final, butunleme) VALUES (?,?,?,?,?)", (ders, tarih, vize, final, but))
    veritabani.commit()
    sinavlari_goster()
...
def sinav_guncelle():
    ...
    if secili:
        ...
        db.execute("UPDATE sinavlar SET ders=?, tarih=?, vize=?, final=?, butunleme=? WHERE id=?",
                   (ders_alani.get(), takvim.get_date(), vize_alani.get(), final_alani.get(), but_alani.get(), kayit_id))
        veritabani.commit()
        sinavlari_goster()
        sinav_takvimi_goster()
...
def kayit_sec(event):
    ...
    if secili:
        ...
        but_alani.delete(0, tk.END)
        but_alani.insert(0, kayit[5])
...
sinav_takvimi.bind("<<CalendarSelector>>", lambda e: messagebox.showinfo("Sınavlar",
